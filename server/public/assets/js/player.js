var Player = {
  ...
  now_playing_listener: function(obj) {
    ...
    if (obj.np != undefined) {
      ...
      if (
        offline &&
        (video_id == "" || video_id == undefined || local_new_channel) &&
        !client
      ) {
        ...
        videoSource = obj.np[0].hasOwnProperty("source")
          ? obj.np[0].source
          : "youtube";
    ...
    if (obj.np != undefined && !offline) {
      ...
      if (
        ((embed && autoplay) || !embed) &&
        (!was_stopped || buffering) &&
        !client
      ) {
        Helper.log([
      ...
      try {
        if (videoSource == "soundcloud" && full_playlist != undefined) {
          document.getElementById("thumbnail_image").innerHTML =
            ...
            full_playlist[full_playlist.length - 1].thumbnail +
            "' alt='thumbnail' />";
        } else {
          document.getElementById("thumbnail_image").innerHTML =
            "<img id='thumbnail_image_channel' src='https://img.youtube.com/vi/" +
            video_id +
            "/mqdefault.jpg' alt='thumbnail' />";
        }
      } catch (e) {}
    }
  },
  ...
  onPlayerStateChange: function(newState) {
    Helper.log(["onPlayerStateChange", "New state\nState: ", newState.data]);
    if (
      Player.player &&
      Player.player.getCurrentTime() > startTime + Player.np.start &&
      !fix_too_far &&
      autoplay
    ) {
    ...
    try {
      Helper.log([
        ...
        "Current time: " + Player.player.getCurrentTime()
      ]);
      Helper.log([
        "getVideoUrl(): " + Player.player.getVideoUrl().split("v=")[1]
      ]);
    } catch (e) {}
    ...
    if (chromecastAvailable) {
      ...
      if (videoSource == "soundcloud") {
        if (!soundcloud_enabled) {
          console.error(
            "SoundCloud isn't enabled, so can't search on SoundCloud.."
          );
          return;
        }
  ...
  loadSoundCloud: function(id, this_duration, start, end, _autoplay) {
        ...
        .then(function(player) {
          ...
          SC_player.get("/tracks", {
            ...
            .then(function(tracks) {
              ...
              document.querySelector("#soundcloud_listen_link").href =
                sound.permalink_url;
    ...
    try {
      Helper.css(
        "#player_overlay",
        "background",
        "url('" + Player.np.thumbnail + "')"
      );
    } catch (e) {
      console.error(
        "Woops this seems to be the first song in the channel. This will be fixed.. soon.. we think.."
      );
    }
  ...
  initializeSCWidget: function(_autoplay, id) {
    ...
    if (document.querySelectorAll("#scplayerElement").length == 0) {
      ...
      document.querySelector("#sc_player").innerHTML =
        '<iframe id="scplayerElement" style="opacity:0;" width="100%" height="166" scrolling="no" frameborder="no" allow="autoplay;" \
      ...
      Player.soundcloud_player.bind("play", Player.soundcloudPlay);
      Player.soundcloud_player.bind("error", Player.soundcloudError);
      Player.soundcloud_player.load(
        "https://api.soundcloud.com/tracks/" + id + this_autoplay,
        { single_active: false }
      );
    }
  },
  ...
  addSCWidgetElements: function() {
    try {
      Player.soundcloud_player = SC_widget.Widget(
        document.querySelector("#scplayerElement")
      );
  ...
  loadVideoById: function(id, this_duration, start, end) {
    ...
    if (chromecastAvailable) {
      ...
      chrome.cast.media.GenericMediaMetadata({
        ...
        images: ["https://img.youtube.com/vi/" + id + "/mqdefault.jpg"]
      });
  ...
  playNext: function() {
    ...
    videoSource = next_song.hasOwnProperty("source")
      ? next_song.source
      : "youtube";
    var start;
    var end;
  ...
  sendNext: function(obj) {
    ...
    if (embed) {
      if (window.parentWindow && window.parentOrigin) {
        window.parentWindow.postMessage(
          { type: "nextVideo", title: obj.title },
          window.parentOrigin
        );
      }
    }
  ...
  soundcloudReady: function() {
    ...
    if (videoSource == "soundcloud" && video_id != undefined)
      Player.loadVideoById(
        video_id,
        Player.np.duration,
        Player.np.start,
  ...
  setBGimage: function(c) {
    ...
    if (
      ...
      ((offline && c.only) || (!offline && !c.only) || (!offline && c.only))
    ) {
      document.getElementById(
        "main-container"
      ).style.backgroundColor = Helper.rgbToHsl(color, true);
      Helper.css("#nav", "background-color", Helper.rgbToHsl(color, true));
      Helper.css(
        ".title-container",
        "background-color",
        Helper.rgbToHsl(color, true)
      );
      var hexHsl = Helper.rgbToHex(color[0], color[1], color[2]);
      try {
        var hsl = Helper.rgbToHsl(color, true)
          .replace("hsl(", "")
          .replace(")", "")
          .replace("%", "")
          .replace(/ /g, "")
    ...
    if (!Helper.mobilecheck()) {
      if (window.innerWidth > 769) {
        var test_against_width =
          window.innerWidth -
          Helper.computedStyle(".control-list", "width") -
          document.querySelector(".zbrand").offsetWidth -
          document.querySelector(".brand-logo-navigate").offsetWidth -
          66;
        title_width = test_against_width;
        document.querySelector(".title-container").style.width =
          title_width + "px";
      } else {
  ...
  notifyUser: function(id, title) {
    ...
    try {
      if (
        ...
        if (videoSource)
          icon = full_playlist[full_playlist.length - 1].thumbnail;
      ...
      if (!user_auth_avoid && duration !== undefined) {
        ...
        if (videoSource == "soundcloud") {
          if (scUsingWidget) {
            ...
            currDurr =
              Math.floor(Player.soundcloud_player.currentTime()) / 1000;
          }
        } else {
          currDurr =
            Player.player.getCurrentTime() !== undefined
              ? Math.floor(Player.player.getCurrentTime())
              : seekTo;
        }
        ...
        if (embed) {
          if (window.parentWindow && window.parentOrigin) {
            window.parentWindow.postMessage(
              {
                type: "duration",
                duration: Player.player.getCurrentTime() - Player.np.start,
                full_duration: Player.player.getDuration() - Player.np.end,
                percent: per
              },
              window.parentOrigin
            );
        ...
        if (videoSource == "soundcloud") {
          if (scUsingWidget) {
            ...
            if (
              Math.ceil(currDurr) + 1 > Player.np.end &&
              Player.soundcloud_player.isPlaying()
            ) {
              end_programmatically = true;
              if (!offline) {
                Player.soundcloud_player.pause();
          ...
          if (
            Math.ceil(Player.player.getCurrentTime()) >= Player.np.end &&
            Player.player.getPlayerState() == YT.PlayerState.PLAYING
          ) {
            end_programmatically = true;
            if (!offline) {
            ...
            Math.ceil(Player.player.getCurrentTime()) < Player.np.end &&
            Player.player.getState() != YT.PlayerState.PLAYING &&
            !was_stopped
          ) {
            Player.player.playVideo();
          }
  ...
  loadPlayer: function(notify) {
    if (
      ...
      try {
        ...
        console.error(error);
        console.error(
          "Seems YouTube iFrame script isn't correctly loaded. Please reload the page."
        );
      }
    } else {
  ...
  loadSoundCloudPlayer: function() {
    if (
      document.querySelectorAll(
        "script[src='https://connect.soundcloud.com/sdk/sdk-3.3.0.js']"
      ).length == 1
    ) {
      try {
        if (
      ...
      if (tagSC.readyState) {
        ...
        tagSC.onreadystatechange = function() {
          if (tagSC.readyState == "loaded" || tagSC.readyState == "complete") {
            tagSC.onreadystatechange = null;
            SC_player = SC;
            if (
              SC_player != null &&
              SC_player != undefined &&
              ((SC_widget != null && !client) || client) &&
              ((SC_widget != undefined && !client) || client) &&
              !sc_initialized
            ) {
              SC_player.initialize({
                client_id: api_key.soundcloud,
  ...
  loadSoundCloudIframe: function() {
    if (
      document.querySelectorAll("script[src='/assets/sclib/scapi.js']")
        .length == 1
    ) {
      try {
        if (
      ...
      if (tagSCWidget.readyState) {
        //IE
        tagSCWidget.onreadystatechange = function() {
          if (
            tagSCWidget.readyState == "loaded" ||
      ...
      firstScriptTagSCWidget.parentNode.insertBefore(
        tagSCWidget,
        firstScriptTagSCWidget
      );
    }
  }
};
